...
const ALLOWED: Record<string, string> = {
  "00837": "is_submission_photo_path — the same mistake as 00711 and 00724, shipped on 2026-09-26 and APPLIED to prod before this gate was run. Superseded by 00846, which makes the function SECURITY INVOKER and restores the default EXECUTE, so the submissions SELECT policies scope it instead. Listed because the text stays in an applied migration, NOT because the pattern is acceptable (2026-09-26)",
  "00724": "pollable_ebay_owner_ids — the same mistake as 00711 and 00685, shipped on 2026-09-03 and APPLIED to prod before this gate was next run. Superseded by 00726, which restores the default EXECUTE and moves the service-role check into the function body. That check has to stay: the function returns other tenants' owner ids by design, so reaching it as a signed-in seller would be a tenant-isolation break, not just a leak. Listed because the text stays in an applied migration, NOT because the pattern is acceptable (2026-09-04)",
