...
// DEV-01: credits are only spent by a key carrying a monthly quota, and nothing
// sets one yet. The card stays hidden until the usage route says a quota
// exists, so nobody pays for a balance nothing draws down. The balance is the
// workspace OWNER's wallet, read by the edge, because keys debit the owner.
export function ApiOverageCard() {
  const [pending, setPending] = useState<string | null>(null);
  const { data } = useApiUsage();

  if (!data?.overage.quota_enabled) return null;
  const balance = data.overage.balance;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Balance: <span className="font-medium text-foreground">{balance.toLocaleString()} credits</span>.
