...
function getDaysListed(listing: ListingRow): number {
  // US-2727: a draft has no listed_at. This is reached through an is_active
  // filter today, so this is not a live NaN; it is the guard that keeps it
  // that way when someone reuses the helper without that filter.
...
// There is no per-item InventoryItemSuggestions component here any more.
// It rendered this file's engine for ONE item on src/pages/inventory-detail.tsx,
// a page that no longer exists; its successor, flipdesk/item.tsx, never adopted
// it. US-2436 removed the orphan. The engine above and the dashboard surface
// ListingSuggestions are both live and unchanged.
