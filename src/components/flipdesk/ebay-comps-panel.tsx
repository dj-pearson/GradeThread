...
import { type GradeBandedPrice, useEbayComps, useGradeBandedPrice } from "@/hooks/use-ebay";
...
// US-594: the sold-comp, grade-banded recommendation. Realized sales win; an
// active-ask fallback is clearly flagged as an estimate. Always surfaces the
// comp set + confidence + sell-through behind the number.
...
function SoldCompRecommendation({
  recommendation,
  loading,
  onApply,
}: {
  ...
  if (!recommendation || !recommendation.sufficient || recommendation.recommendedCents == null) {
    return null;
  }

  const { recommendedCents, lowCents, highCents, currency, basis, soldBacked, confidence, compSet, sellThrough } =
    recommendation;
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px] text-muted-foreground sm:grid-cols-3">
        ...
        <div>
          ...
          <div className="text-sm font-medium text-foreground">{confidencePct}%</div>
        </div>
        <div>
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        ...
        <Button variant="outline" size="sm" onClick={() => onApply(dollars, currency)}>
          Use as target price
        </Button>
      </div>
    </div>
