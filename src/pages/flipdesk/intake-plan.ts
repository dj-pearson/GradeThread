 ...
 * get_or_create_source); "__new" with a blank name is refused before this is
 * reached, and reads as none here so a stray blank never creates a source.
 */
export function resolveIntakeSource(
  form: Pick<IntakeFormState, "source_id" | "source_new">,
): IntakeSourceChoice {
  if (form.source_id === "__new") {
    const name = form.source_new.trim();
    return name ? { kind: "new", name } : { kind: "none" };
  }
  if (form.source_id && form.source_id !== "__none") {
    return { kind: "existing", id: form.source_id };
  }
  return { kind: "none" };
}
