...
describe("send-offer arithmetic and wording (OM-12)", () => {
  ...
  it("reports a partial send and keeps the failures", () => {
    ...
    expect(out.message).toBe("Sent to 3 of 4 listings. The rest are still selected. Not eligible.");
    expect(describeSendResult({ ok: true, count: 1 }, 1).message).toBe("Offer sent on 1 listing.");
  });
