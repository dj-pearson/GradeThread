...
if (import.meta.main) {
  ...
  if (!url || !serviceKey) {
    console.error(
      "seed-registered-numbers: SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required. Usage:\n" +
        "  deno run --allow-net --allow-env --allow-read scripts/seed-registered-numbers.ts [--apply] [--limit N]",
    );
    Deno.exit(1);
  }
  await main();
