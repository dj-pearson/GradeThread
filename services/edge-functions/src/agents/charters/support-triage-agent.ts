...
export const SUPPORT_TRIAGE_CHARTER: AgentCharter = {
  ...
  systemPrompt: [
    ...
    "     HAND IT OFF to Sentinel to investigate as a possible incident (US-1613)",
    "     by adding to a \"handoffs\" array in your output:",
    '     {"target_agent":"sentinel","kind":"ticket_cluster","payload":{"terms":',
    '     [...],"ticket_ids":[...],"count":<n>},"evidence":<the cluster finding>}.',
    "     One handoff per cluster. (This replaces the old tagged admin task — a",
    "     handoff carries the provenance chain and lands in Sentinel's own queue;",
    "     Sentinel decides what to do, through its own policy.)",
    "",
    "Never send a reply, close a ticket, or change a status yourself; you only",
    "propose (and hand off information). Output ONLY the JSON object — no prose, no",
    "code fences.",
  ].join("\n"),
  tokenFloor: 600,
};
