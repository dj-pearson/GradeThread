...
// Returns a valid Depop access token, refreshing it when it is within
...
// depop rotates and invalidates the old one, so the loser took an
...
export function getDepopAccessToken(
  userId: string,
  ...
  return singleFlightRefresh(
    `depop:${userId}`,
    () => getDepopAccessTokenUncoordinated(userId, within),
  );
}

async function getDepopAccessTokenUncoordinated(
  userId: string,
  ...
  try {
    ...
    if (permanent) {
      ...
      if (
        ...
        console.warn(
          `[depop-client] refresh race for user ${userId}; using the token a sibling stored`,
        );
...
export async function refreshExpiringDepopConnections(
  ...
  for (const userId of userIds) {
    try {
      await getDepopAccessToken(userId, { refreshWithinMs: horizonMs });
      refreshed += 1;
    } catch (err) {
      failed += 1;
      console.error(
