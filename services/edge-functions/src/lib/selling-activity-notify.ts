...
//              (default ON). sale_recorded / payout_imported / listing-ended
//              (US-1977, category "listing.ended") carry a push; listing-live has
//              no iOS category so it stays in-app only.
//
...
import { pushListingEnded, pushPayoutCleared, pushSaleCreated } from "./transactional-push.ts";
...
/**
 * A listing ended without selling and returned to drafts. US-1977: also emits an
 * APNs push (category "listing.ended"), gated by the same selling_activity
 * preference as the in-app leg. Fires for both ended-unsold and ended-by-eBay
 * detection — the offers sync funnels both into the Path-A reconcile that calls
 * this on the real listed→drafted transition.
 */
