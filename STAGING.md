...
1. **Supabase:** provision a second self-hosted Supabase (own Postgres, own
   ...
   all of `supabase/migrations/` (see `MIGRATIONS.md`). Create the
...
2. Merge to `staging` → Pages builds `staging.gradethread.com`; Coolify
   ...
   first (`MIGRATIONS.md` — same order rule as prod: DB → edge → frontend).
...
4. Green → merge `staging` into `main` → prod deploy (`DEPLOY.md`), then the
   prod post-deploy checks (`LAUNCH_CHECKLIST.md` §6).
