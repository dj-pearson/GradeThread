...
-- -- Split by HOW the grade was paid. refund_grade() writes an INCLUDED
-- -- refund as a delta-0 row: the grade goes back to the monthly bundle
-- -- (users.grades_used_this_month - 1), not to the credit balance. So
-- -- credits_returned = 0 on an included row is the refund WORKING. Only a
-- -- credits row with credits_returned 0 would mean a seller was not repaid.
SELECT
