...
-- ════════════════════════════════════════════════════════════════════
-- §29 US-2282 — CAN AN ANONYMOUS CALLER MINT CREDITS? (run this first)
-- ════════════════════════════════════════════════════════════════════
...
-- -- anon_can_run = t on any row here means the exploit is live.
SELECT p.proname,
...
-- -- checks internally. The credit functions do not check. So (c) is a list to
-- -- triage, not a count of vulnerabilities; (a) is the one that is already
-- -- triaged.
-- --
-- -- THE FIX IS DEADLOCKED and the order matters: 00527 revokes exactly these
-- -- grants and is parked because revoking is what arms the US-2403 segfault.
-- -- Clear supautils.hint_roles on the host FIRST, then apply 00527. The
-- -- smaller interim fix is an internal authorization check inside the credit
-- -- functions, the way admin_revenue_metrics already has one.
