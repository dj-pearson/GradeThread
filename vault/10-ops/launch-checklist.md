...
| ebay-payout-link | `30 5 * * *` | `/api/jobs/ebay-payout-link` | `$FLIPDESK_INTERNAL_JOB_SECRET` | 200 {owners,eligible_owners,payouts_upserted,sales_linked,sales_still_unlinked,failed_owners}; sales_linked falls toward 0 once the backlog drains and sales_still_unlinked is normal — those are orders whose deposit has not settled yet. skipped:true with reason ebay_not_configured is healthy in an env with no keyset |
...
| extension-queue-stale | `50 16 * * *` | `/api/jobs/extension-queue-stale` | `$FLIPDESK_INTERNAL_JOB_SECRET` | 200 with {ok:true, scanned, candidates, notified, suppressed, skipped{...}}; notified is 0 on most days and `capped:true` means the scan hit its row ceiling and under-reported |
...
| grade-release | `*/5 * * * *` | `/api/jobs/grade-release` | `$FLIPDESK_INTERNAL_JOB_SECRET` | 200 with {ok:true, hold_enabled, released, preliminary_notices, failures:[]}; released and preliminary_notices are 0 while the hold is off |
...
| measurement-aggregate | `40 3 * * *` | `/api/jobs/measurement-aggregate` | `$FLIPDESK_INTERNAL_JOB_SECRET` | 200 with {ok:true, cohorts, sufficient, upserted, retired}; every count can be 0 before any garment is measured |
| measurement-text-backfill | `*/15 * * * *` | `/api/jobs/measurement-text-backfill` | `$FLIPDESK_INTERNAL_JOB_SECRET` | ONE-OFF at launch (idempotent; disable once drained); 200 with {ok:true, scanned, withMeasurements, ingested, drained}; scanned 0 with drained:true once the backlog is read |
