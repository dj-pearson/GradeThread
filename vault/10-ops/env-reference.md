...
> for `8c8d2d712` (US-3107): a comment over `EBAY_MARKETPLACE_INSIGHTS` saying
> that flipping it is half the change (the scope must be in `EBAY_SCOPES`, and
> `src/test/asking-price-copy.test.ts` fails until the asking-price copy is
> reworded), plus `SCOUT_EBAY_IMAGE_SEARCH_ENABLED=false`. Both variables are
> already in the table below and the rollout checklist; nothing to carry.
...
| `PLATFORM_VARIANT_AI_MODEL` | ⬜ Coolify edge | Text model for the cross-list copy kit pass (Poshmark/Mercari/Depop/Grailed/Vinted copy adapted from the eBay draft). Defaults to `getLightweightModel()` since 2026-09-02, when the kit started being filled with every AutoLister draft; it ran on `getDefaultModel()` before. Text in, text out, facts pinned to the source, so its own knob exists only for rollback (`claude-sonnet-5`) without moving every other lightweight caller (`lib/ai-config.ts` `getPlatformVariantModel`). |
...
| `GRADING_SCHEMA_IN_SYSTEM` | ⬜ Coolify edge | US-3150. Default **OFF**. On, the per-image and composite response schema plus rules move from the user turn into a second cached system block, and both version stamps gain `+sysschema`. Off is byte-identical to the shipped request body. ⚠ Do NOT turn this on before the per-image fan-out is staggered: grading fires every photo call at once, so photos 2..N cannot read photo 1's cache write and each pays the 1.25x premium instead. As shipped that is a **25 percent loss** per grade on that text, not a saving. The stagger now exists as `GRADING_CACHE_STAGGER` below (default off): turn that on first, or this is a loss. |
| `GRADING_LEGIBLE_V2` | ⬜ Coolify edge | US-3321. Default **OFF**. On, the per-image Rules block defines a label as legible when the marking that IS on the tag can be read, and names the tagless cases (heat-transfer print, silicone size dot, waistband stamp, size-only tag), so a sharp photo of one stops carrying `ILLEGIBLE_LABEL_CONFIDENCE_CAP`. Off is byte-identical. Stamps `+legible2` on the per-image entry and on the grade. A prompt-text change: shadow, the golden-set eval gate and a canary run with it on before it goes on for everyone. |
| `GRADING_SCALE_REFERENCE` | ⬜ Coolify edge | Default **OFF**. On, the per-image prompt tells the model to size a flaw against a US quarter (24.26 mm), a bank or ID card (85.60 x 53.98 mm) or a MeasureCard laid beside it, and never to record that object as a flaw (US-3332). Off is byte-identical. Stamps `+scale` on the per-image entry and on the grade. A prompt-surface change, so it rides the shadow/eval/canary lane like any other. See [[grading-prompt-channels]]. |
| `GRADING_REFERENCE_ANCHORS` | ⬜ Coolify edge | Default **OFF**. On, the composite call carries up to `GRADING_REFERENCE_ANCHORS_MAX` awarded reference photos for the item's category (US-3334's gallery), each labeled "NOT this garment" with its awarded value (US-3335). **The flag alone does nothing**: anchors serve only after the reference-photo eval on the admin grading page has PASSED (system setting `reference_anchor_eval`). Never the item's own photos; owner consent re-checked at use. Stamps `+anchors` on the grade; anchor image tokens are metered on their own `composite_reference_anchors` phase. See [[grading-prompt-channels]]. |
| `GRADING_REFERENCE_ANCHORS_MAX` | ⬜ Coolify edge | How many reference photos ride along when anchors are on. Default 3, capped at 4. Each is a full image (about 1,600 input tokens) on every grade. |
| `GRADING_FABRIC_ZOOM` | ⬜ Coolify edge | Default **OFF**. On, every non-Forensic grade re-reads the fabric close-up (the `detail:fabric` slot, else the first detail shot) as up to 4 tiles near native resolution and merges what they see through the Forensic zoom-merge path; new flaws are added only when pinhole or small, at most 5 (US-3338). Moves grades, so treat it as a grading change. Forensic grades keep their own zoom unchanged. Stamps `+fabriczoom` on the close-up's read and on the grade; tile calls are metered on the `per_image_fabric_zoom` phase. |
| `GRADING_FABRIC_ZOOM_DAILY_CAP_USD` | ⬜ Coolify edge | Daily (UTC) spend cap for the fabric zoom, summed from `ai_usage_events` on the `per_image_fabric_zoom` phase. Default 5. At or over the cap the pass is skipped and grades run as if the flag were off; `0` turns it off. A `system_settings` row `grading_fabric_zoom_daily_cap_usd` overrides it without a redeploy. If the ledger cannot be read the pass is skipped (fails closed). |
| `GRADING_CARD_SIZING` | ⬜ Coolify edge | Default **OFF**. On, enables card-defect size bucketing. ⚠ A bucket change MOVES THE DEFECT WEIGHT, so this alters grades: treat it as a grading change, not a display toggle. |
...
| `GRADING_CLEANLINESS_V2` | ⬜ Coolify edge | Describes the fifth factor to the model as VISIBLE cleanliness only (stains, soiling, yellowing, sweat and deodorant marks, pet hair, lint, residue) and tells it smell cannot be judged from a photo (US-3329). **Default OFF, and off means byte-identical prompts.** Suffixes `+clean2` on both the per-image entry and the grade. Touches the per-image and composite system prompts and the composite factor-weights sentence; the factor key and weight do not change. See [[grading-scale-and-weights]]. |
...
| `GRADING_SELF_CONSISTENCY_SAMPLE` | ⬜ Coolify edge | **Costs money when set.** Submissions per weekly reproducibility run (US-2035); each is regraded `GRADING_SELF_CONSISTENCY_RUNS` times, so the bill is sample x runs x images. **Unset or 0 = the job no-ops**, which is the default. Capped at 25 in code whatever this says. Start at 3. Each run also stores its measured spreads per garment category (system setting `grading_consistency_by_category`), and a category with 10 or more measurements in the last 180 days gets a "likely X to Y" range on its certificates and seller reports (US-3339; 80th percentile of the spread, never from confidence). The sample is recent grades of any category, so a rare category takes many runs to reach 10. |
...
| `AI_TIMEOUT_MS` / `AI_MAX_RETRIES` / `AI_ENABLE_CACHING` | ⬜ Coolify edge | Per-call timeout, retry cap, and prompt-cache toggle. Read by `lib/ai-config.ts`. `AI_ENABLE_CACHING` is shared by ten features (extract, authenticity, listing, the agent kernel and friends), several of which are sequential loops whose caches really do read — so do not turn it off to deal with grading. Use `GRADING_ENABLE_CACHING` for that. |
| `GRADING_CACHE_STAGGER` | ⬜ Coolify edge | US-3345. Default **OFF**, and inert unless grading caching is on. On, the per-image fan-out (first pass and escalation) sends photo 1 STREAMED and releases photos 2..N at its first token, so they read the system-prompt cache entry photo 1 wrote instead of each paying the 1.25x write. Costs photo 1's time to first token once per grade; every grade logs it as `gate_ms` on the `[Pipeline] per-image fan-out` line, beside `stagger_ms` (photo 1's whole call). Transport and timing only: no byte the model reads changes, no `prompt_version` suffix. If `gate_ms` is not worth the reads, turn this off and `GRADING_ENABLE_CACHING` off with it. |
| `GRADING_ENABLE_CACHING` | ⬜ Coolify edge | US-3345. Prompt caching for the **grading path only** (per-image and composite system blocks). Unset or empty **inherits `AI_ENABLE_CACHING`**, so the default request body is unchanged. It exists because grading fires every per-image call at once: a cache entry is readable only once the request writing it has begun streaming, so photos 2..N cannot read photo 1's write, each pays the 1.25x premium, and the composite is a single call with nothing to read. If `ai_usage_events` shows grading writes with no reads (the SQL is in the `gradingCachingEnabled` doc comment in `lib/ai-config.ts`), set this to `0` and stop paying the premium. **Not a prompt change** — `cache_control` alters no byte the model reads, so it earns no `prompt_version` suffix and does not go through shadow → eval → canary. |
...
| `EBAY_EPN_CAMPAIGN_ID` | ⬜ Coolify edge | eBay Partner Network campaign id (10 digits). When set, Reseller Swap tip links carry EPN tracking so eBay pays GradeThread a commission on a qualifying purchase. Unset or malformed means plain eBay links; the feature still works (`lib/ebay-affiliate.ts`, US-3541, [[reseller-swap]]). |
