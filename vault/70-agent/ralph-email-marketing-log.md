---
title: Ralph email-marketing working log
type: learning
status: current
source_of_truth: vault
code_refs: []
reviewed: 2026-07-19
tags: [agent, ralph, email]
summary: Email and deliverability gotchas accumulated by the loop; loaded on demand.
---

> [!info] Loaded on demand, not every iteration
> Split out of the main playbook so its bulk is not re-read every loop — read it
> only when the story touches email and deliverability. That selective-loading pattern is the same
> one the vault index uses, and it predates the vault.
# Ralph Learnings — Email, newsletter & drip

Read this IN ADDITION to LEARNINGS.md when your story touches the newsletter/broadcast/drip/SES surfaces (US-911…US-946 family, jobs-journey-tick, send coordinator, suppression).
...
- [[deliverability]] — the SES/SMTP contract
