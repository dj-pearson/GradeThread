-- US-3536: the Finances dashboard used different tier bands from the grade.
--
-- finances_dashboard (00143) labelled each sale's grade tier with floors of
-- 9.5 / 8.5 / 7.5 / 6.5 / 5.5 / 4.5, while every certificate, the edge
-- (scoreToGradeTier) and the web (GRADE_TIER_BANDS) use 10 / 9 / 8 / 7 / 6 / 5.
-- So an 8.7 grade was "Excellent" on its certificate and "NWOT" in the seller's
-- tier profit stats. This re-creates the function from 00143 with ONLY that
-- CASE changed; CREATE OR REPLACE keeps the existing grants.
...
base as (
  select
    ...
    case
      -- US-3536: the real tier floors (GRADE_TIER_BANDS / scoreToGradeTier).
      when i.grade_value >= 10 then 'NWT'
      when i.grade_value >= 9 then 'NWOT'
      when i.grade_value >= 8 then 'Excellent'
      when i.grade_value >= 7 then 'Very Good'
      when i.grade_value >= 6 then 'Good'
      when i.grade_value >= 5 then 'Fair'
      else 'Poor'
    end as grade_tier
  from public.sales s
...
insert into public.applied_migrations (version) values ('00844') on conflict do nothing;
