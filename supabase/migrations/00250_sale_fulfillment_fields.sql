-- 00250_sale_fulfillment_fields.sql
--
-- US-960: Shipped-tab fulfillment. The FlipDesk ship flow records fulfillment on
-- the sales row (sales.shipped_at + sales.tracking_number already exist from
-- 00008). Add the two missing fields — `carrier` and `delivered_at` — so a
-- reseller can record the carrier and mark an order delivered from the Shipped
-- tab, then surface carrier/shipped_at/delivered_at on items_full so the
-- listings table can show them without a second query.
--
-- RLS: no new policy needed. sales RLS (00002) is row-level (scoped through
-- inventory_items.user_id), so the new columns inherit the existing
-- view/insert/update/delete policies automatically. items_full stays
-- security_invoker (see 00033/00249), so the joined sale columns are only
-- visible for the caller's own rows.
--
-- CREATE OR REPLACE VIEW forbids reordering/renaming existing columns, so every
-- column below is reproduced in its exact existing position (see 00249); the
-- only change is the three NEW columns APPENDED at the very end (allowed by
...
SELECT
  ...
  -- US-960: NEW trailing columns (appended for CREATE OR REPLACE compat) —
  -- Shipped-tab fulfillment surfaced from the most-recent sale.
