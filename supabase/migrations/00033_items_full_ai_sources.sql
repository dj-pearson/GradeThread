-- Expose ai_field_sources on the items_full view so the MeasurementForm
-- (and any other surface that takes ItemFullRow) can render the "AI
-- estimated" badge on per-field provenance — including the prefixed keys
-- `measurements.<field>` set by SnapCatalog when applying AI-suggested
-- brand-spec measurements.
--
-- View recreation preserves existing security_invoker setting + all
...
SELECT
  ...
  -- New: AI provenance map. Includes "measurements.<field>" entries so
  -- the MeasurementForm can render the "AI" badge per-field.
