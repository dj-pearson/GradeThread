-- Extend the items_full view with extra trailing columns the Listings tabs
-- need: buyer_id (repeat-buyer detection), raw sold_at (ship-by countdown),
-- payout_reference (reconciliation state) and listing_status (Active tab).
--
-- CREATE OR REPLACE VIEW requires the new definition to keep every existing
-- column in the same position; new columns may only be appended at the end.
