-- 00249_items_full_color_platform.sql
--
-- US-1051: expose two more columns on the items_full view so the web advanced
-- filter can reach feature parity with iOS — `color` (from inventory_items)
-- and `listing_platform` (the most-recent listing's marketplace). Both are used
-- as facets in the FlipDesk filter builder; without them the web rule builder
-- could offer the fields but never evaluate them against loaded rows.
--
-- CREATE OR REPLACE VIEW forbids reordering/renaming existing columns, so every
-- column below is reproduced in its exact existing position (see 00112); the
-- only change is the two NEW columns APPENDED at the very end (allowed by
-- CREATE OR REPLACE). security_invoker preserved (see 00033). Idempotent.
...
SELECT
  ...
  -- US-1051: NEW trailing columns (appended for CREATE OR REPLACE compat).
  i.color                                                                AS color,
