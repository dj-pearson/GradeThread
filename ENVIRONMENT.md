...
| Access-token lifetime | `GOTRUE_JWT_EXP` | `3600` | Already 3600 in config.toml; keep short — the SDK auto-refreshes it, so this is NOT how long a login lasts. |
| Absolute session TTL | `GOTRUE_SESSIONS_TIMEBOX` | `720h` (30d) or unset | How long a login can live regardless of activity. A SHORT value (12h/24h) is the usual cause of the iOS app "logging out every day" — it revokes the refresh token server-side. `720h` = passive 30-day mobile session; unset = no cap. GoTrue uses Go durations (use `720h`, not `30d`). |
| Session inactivity timeout | `GOTRUE_SESSIONS_INACTIVITY_TIMEOUT` | `720h` (30d) or unset | Logout after this long unused. Same 30-day guidance; keep in sync with the timebox. The web app is bounded separately by a client-side 12h idle-logout (`src/lib/idle-logout.ts`, `scope:"local"`) that never touches the server session. |
