...
- **Rollback:** forward-only. Restore from backup for a catastrophic migration
  (`BACKUPS.md` + `ROLLBACK.md`); otherwise write a compensating migration.
...
- **Scheduled tasks survive a redeploy:** Coolify Scheduled Tasks are configured
  ...
  there are **72** (`CRON_REGISTRY` in `services/edge-functions/src/lib/cron-runs.ts`
...
- **Env vars:** Pages → Settings → Environment variables → **Production** (the
  `VITE_*` set in `LAUNCH_CHECKLIST.md` §1c). Pages env changes only take effect
...
- **Rollback:** Cloudflare Pages → Deployments → **Rollback** to a previous
  deployment (instant, no rebuild). See `ROLLBACK.md`.

---

## 4. Post-deploy verification

Run the smoke checks in **`LAUNCH_CHECKLIST.md` §6** after every production
