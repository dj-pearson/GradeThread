...
- **Android (US-2502):** builds and verifies FULLY on Windows — `npm run verify:android` runs exactly what `Android CI` runs, in the same order (format → detekt → lint → unit tests → coverage floor → screenshots → debug/release/bundle → ABI budget). The pre-push hook adds it automatically when the push touches `android/**`. **First run on a machine: `npm run android:doctor`** — it resolves the JDK (17–23; Gradle 8.13 will not run on 24+, and AGP reports that as the bare string `What went wrong: 25.0.2`), the SDK and Python, and writes `local.properties`. Everything Android Studio does has a command — see the table in `android/README.md` “Working without Android Studio”. Only the Profiler, the LIVE Layout Inspector hierarchy, and breakpoint debugging still need the IDE.
