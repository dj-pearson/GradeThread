...
- **iOS:** can't build/test on Windows (Swift/xcodebuild is macOS-only); `iOS CI` on macOS runners is the safety net. The **ungated-`print` guard runs locally in the web suite** (`src/test/ios-ungated-print.test.ts`, so `npm run verify` covers it with no Python) — it is a port of `ios/Scripts/no-ungated-print.py`, which still runs in `iOS CI`, and a parity case fails if the two scan different trees. The other five `ios/Scripts/*.py` guards (bare strings, force unwrap, shared session, raw JPEG, ATS) still need `python3` and remain CI-only. This line used to say Python was the one thing that ran locally; **there is no `python3` on the Windows dev box**, so the documented local safety net was zero.
