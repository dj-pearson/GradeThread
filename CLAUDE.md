...
- **THE EDGE SUITE RUNS IN A CLOUD SESSION TOO, and it took a month of "no deno binary" notes to check (2026-09-18).** Every edge story written from a Claude Code web session says the tests could not be run. Deno is simply not preinstalled: `curl -fsSL https://github.com/denoland/deno/releases/download/v2.8.0/deno-x86_64-unknown-linux-gnu.zip`, unzip, `chmod +x`, done in about a minute, and 2.8.0 is the version `ci.yml` pins. `deno lint` then passes on 2,126 files with no further work, which flips one `verify:edge` lane from red to green on its own.
  ...
  `deno check` reports 7 errors under this map, all in `payments.ts` / `webhooks.ts` / `r2-client.ts` Stripe and R2 code, all consistent with `npm:stripe@15.5.0` typing `SessionCreateParams` differently from the esm.sh build. **That is not a verdict on the real config** — the substitution is the likely cause and cannot be ruled in or out without esm.sh. Do not act on those 7 from here, and do not record them as clean either.
