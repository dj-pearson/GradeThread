...
- **Two integration lanes are CI-only, deliberately** — `Tenant Isolation` and `Money & Certificate Integration`. Both need the FULL local stack (`supabase start`, not the `db` lane's `supabase db start`) plus a running edge service, which is too slow for every pre-push, so `npm run verify` does NOT mirror them. To reproduce either locally, follow its workflow file step for step: `.github/workflows/tenant-isolation.yml` / `.github/workflows/money-cert-integration.yml`. Neither needs prod credentials — the fixtures are seeded onto the throwaway stack (`services/edge-functions/scripts/seed-money-cert-fixture.ts`), which is also safer, since these suites drain wallets and transfer garment ownership.
