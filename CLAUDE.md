...
- **PostgREST CAN run locally, and several "unverifiable" operator scripts are not (2026-08-16).** `supabase db start` boots Postgres only, and `supabase start` afterwards *stops* kong/rest/studio rather than adding them — so the local stack has no REST API and any script going through `SUPABASE_URL` reads as impossible to execute here. It is not: the CLI leaves those containers **stopped, not removed**, and `docker start supabase_rest_gradethread supabase_kong_gradethread` brings the API up in a second against the same 609-migration schema. That is how `scripts/seed-help-articles.mjs` (all three paths: insert, skip-on-rerun, `--force`), `services/edge-functions/scripts/purge-third-party-subject.ts --find` and `purge-email-subject.ts` were executed for the first time. **Before writing "cannot be verified locally" about anything REST-shaped, try it** — every operator script run for the first time this session had a real defect in it.
