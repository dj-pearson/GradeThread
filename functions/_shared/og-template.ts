...
// ── Font weights: 700 is NOT the problem (US-2619) ─────────────────────────
//
// A previous pass changed the eight font-weight:700 declarations here to 800,
// on a clean-looking correlation: of six templates, the only one that rendered
// in a Pages Function was the only one without a 700.
//
// THAT THEORY IS DISPROVEN. The deployed bundle carries commit 0c24892a (read
// out of /assets/index-*.js on the live site), which is AFTER that change, and
// /og/social/card and /og/verified/<real handle> both still answer with
// X-GT-Fallback: branded. The weights are restored; the correlation was real
// and the causation was not.
//
// ELIMINATED SO FAR, each on evidence rather than opinion: gradients (all six
// use one, including the working one), images (the working one embeds a
// data-URI QR), any CSS PROPERTY present in both failures and absent from the
// success, character set (the failing default text is plain ASCII), module
// depth (a top-level probe rendered the failing template and still fell back),
// and now font weight.
//
// The next step is not another guess from out here: every failure logs its
// exception with the endpoint named, so the Cloudflare Pages log has the
// actual error. That is the OPERATOR criterion on US-2619.
