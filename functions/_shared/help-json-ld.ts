// Help Center structured data (US-2579) — the edge-SSR copy.
//
// ⚠ THIS FILE HAS A TWIN: src/lib/seo/help-json-ld.ts carries the same builders
// for the SPA. They are duplicated rather than shared because functions/ is a
// separate tsconfig project bundled for the Workers runtime, and the repo
// already works this way everywhere else (functions/_shared/blog-render.ts vs
// src/lib/seo/json-ld.ts).
//
// What keeps them honest is src/test/help-json-ld-parity.test.ts, which asserts
// (a) the two file bodies are byte-identical from the first export onward and
// (b) the same article produces deeply equal output from both. Change one and
// that test fails until you change the other.
//
// Dependency-free on purpose: no imports at all, so the twin can stay literally
