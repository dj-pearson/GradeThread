...
- `notifications` (**OPTIONAL** — in `optional_permissions`, never requested at install) — The seller may switch on "instant delisting" from the extension's popup. Chrome gates `registration.pushManager` on this permission, so it is what lets GradeThread's own server wake the background worker the moment one of the seller's cross-listed items sells, instead of the worker finding out on its existing 5-minute check. **The extension shows no notification of any kind, and calls no notifications API** — the subscription is created with `userVisibleOnly: false` and a test fails the build if any shipped file calls `chrome.notifications`. The push carries one field, `{"type":"gt-drain"}`: no listing, no marketplace, no address, no personal data. On receiving it the worker re-reads the seller's own GradeThread job queue, exactly as it already does on the timer. A seller who never grants it loses nothing but the seconds; the 5-minute check still ends their duplicate listings.
