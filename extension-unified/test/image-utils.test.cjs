...
// rejects .cjs content scripts). The repo is type:module, so require() of a .js
// throws ERR_REQUIRE_ESM; load it the same way the config guards load selectors.js
// — run its source with an injected `self` and read the global it assigns.
function loadImageUtils() {
  const src = fs.readFileSync(path.resolve(__dirname, "..", "research", "image-utils.js"), "utf8");
  const selfObj = {};
